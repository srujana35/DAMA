...
def layer_stats(
    ...
    def get_ds():
        ...
        # raw_ds = load_dataset(
        #     "wikitext",
        #     dict(wikitext="wikitext-2-raw-v1")[ds_name],
        #     download_mode="force_redownload", split="train"
        # )
    ...
    npos = model.config.max_position_embeddings  # Number of positions in the model
    ...
    with torch.no_grad():
        for batch_group in progress(loader, total=batch_count):
            for batch in batch_group:
                batch = dict_to_(batch, "cuda" if torch.cuda.is_available() else "cpu")
                with Trace(
